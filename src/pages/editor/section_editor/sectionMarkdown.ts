    ...
    .map((block) => {
      ...
      if (block.type === 'gallery') {
        const headerParts = [
          ...
          `align=${block.align ?? 'height:1'}`,
        ];
        if (block.mode === 'faithful') {
          headerParts.push('mode=faithful');
        }
...
export function parseSectionMarkdown(markdown: string): ArticleBlock[] {
  ...
  while (index < lines.length) {
    ...
    if (line.startsWith(':::gallery')) {
      ...
      const modeMatch = line.match(/mode=(default|faithful)/);
      ...
      blocks.push({
        ...
        mode: modeMatch ? (modeMatch[1] as Extract<ArticleBlock, { type: 'gallery' }>['mode']) : undefined,
      });
      index += 1;
      continue;
    }
